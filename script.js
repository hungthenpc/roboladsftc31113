...
// Thêm vào script.js
// Animation for team section when scrolling
...
function updateActiveMenuOnScroll() {
  // Lấy tất cả sections cần theo dõi
  const sections = {
  ...
  // Tính toán section nào đang visible nhất
  ...
  menuLinks.forEach(link => {
    const section = link.getAttribute('data-section') || 
                   (link.getAttribute('href') && 
                    link.getAttribute('href').includes('ftc.html') ? 'ftc' : '');
    
    link.classList.remove('active');
    
    if (section && 
        (section === currentActive || 
         (section === 'ftc' && window.location.pathname.includes('ftc.html')))) {
...
// ========== SLIDER VỚI HIỆU ỨNG SLIDE LIỀN MẠCH ==========
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function showSlide(index) {
    ...
    setTimeout(() => {
      ...
      isAnimating = false;
    }, 600); // Thời gian phải khớp với CSS transition
  }
